...
def generate_random_entry() -> tuple[str, str, str]:
    website = 'www.' + ''.join(choice(ascii_letters + digits) for _ in range(10)) + '.com'
    username = ''.join(choice(ascii_letters + digits) for _ in range(10))
    password = generate_password(16)
    return website, username, password

def make_test_users(database: Database, number_of_users: int, number_of_entries: int) -> None:    
    for i in range(number_of_users):
        username, hashed_password, symmetrical_key = generate_user(f"username{i}", "password")
        ...
        for _ in range(number_of_entries):
            ...
            encrypted_password = encryption.encrypt_password(password, symmetrical_key)

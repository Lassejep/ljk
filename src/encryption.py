...
def encrypt_symmetrical_key(symmetrical_key: bytes) -> bytes:
    ...
    encrypted_symmetrical_key = public_key.encrypt(
        symmetrical_key, asym_pad.PKCS1v15()
    )
...
def decrypt_symmetrical_key(encrypted_symmetrical_key: bytes) -> bytes:
    ...
    symmetrical_key = private_key.decrypt(
        encrypted_symmetrical_key, asym_pad.PKCS1v15()
    )
...
def decrypt_password(encrypted_password: bytes, key: bytes) -> str:
    ...
    padded_password = decryptor.update(
        encrypted_password[block_size // 8:]
    ) + decryptor.finalize()
    # Unpad the password
